//
//  ZhuBoAAAViewController.m
//  喜马拉雅FM
//
//  Created by 李东 on 16/11/23.
...
@interface ZhuBoAAAViewController () <UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation ZhuBoAAAViewController
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZhuBoMoreList *list = self.dataSource[indexPath.row];
...
-(void)netWorkRequest:(NSString *)urlString{
   ...
   [AFNetWorking_Request getRequestWithUrl:urlString Result:^(id result) {
       ZhuBoMoreZhuBoMoreModel *model = [ZhuBoMoreZhuBoMoreModel modelObjectWithDictionary:result];
       
       NSArray *array = [NSArray arrayWithArray:model.list];
       
       
       for (ZhuBoMoreList *list in array) {
